...
export default async function ChocolateriePage({ searchParams }: { searchParams: Promise<{ page?: string; q?: string }> }) {
        ...
        <main className="bg-background text-foreground">
            <div className="mx-auto w-full max-w-6xl px-6 py-10 sm:py-20 sm:px-10 lg:px-12">
                <div className="relative flex flex-col gap-4 justify-center items-center">
                    <h1 className="absolute top-10 sm:top-18 text-2xl sm:text-4xl font-text uppercase text-secondary z-10">Chocolaterie</h1>
                    <span className="font-title text-accent/70 text-7xl sm:text-9xl text-center drop-shadow-lg z-0">Chocolaterie</span>
            ...
            <ProductsSection category="chocolaterie" page={Number.isNaN(page) ? 1 : page} query={params.q} />
        </main>
    );
}
